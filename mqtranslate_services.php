...
// error messages
$qs_error_messages[QS_ERROR_INVALID_LANGUAGE] =			__('The language/s do not have a valid ISO 639-1 representation.','mqtranslate');
$qs_error_messages[QS_ERROR_NOT_SUPPORTED_LANGUAGE] =	__('The language/s you used are not supported by the service.','mqtranslate');
$qs_error_messages[QS_ERROR_INVALID_SERVICE] =			__('There is no such service.','mqtranslate');
$qs_error_messages[QS_ERROR_INVALID_ORDER] =			__('The system could not process your order.','mqtranslate');
$qs_error_messages[QS_ERROR_SERVICE_GENERIC] =			__('There has been an error with the selected service.','mqtranslate');
$qs_error_messages[QS_ERROR_SERVICE_UNKNOWN] =			__('An unknown error occured with the selected service.','mqtranslate');
$qs_error_messages[QS_DEBUG] =							__('The server returned a debugging message.','mqtranslate');
...
add_filter('mqtranslate_configuration_pre',		'qs_config_pre_hook');
...
function qs_translateButtons($available_languages, $missing_languages) {
	...
	foreach($missing_languages as $language) {
		$missing_languages_name[] = '<a href="edit.php?page=mqtranslate_services&post='.$post->ID.'&target_language='.$language.'">'.$q_config['language_name'][$language].'</a>';
	}
	$missing_languages_names = join(', ', $missing_languages_name);
	printf(__('<div>Translate to %s</div>', 'mqtranslate') ,$missing_languages_names);
}
...
#mqtranslate-services h4 { margin-top:0 }
#mqtranslate-services h5 { margin-bottom:0 }
#mqtranslate-services .description { font-size:11px }
...
function qs_load() {
	...
	$mqtranslate_services = get_option('mqtranslate_mqtranslate_services');
	$mqtranslate_services = qtrans_validateBool($mqtranslate_services, $q_config['mqtranslate_services']);
	$q_config['mqtranslate_services'] = $mqtranslate_services && qs_isOpenSSLAvailable();
	if($q_config['mqtranslate_services'] && is_string($qs_public_key)) {
...
function qs_init() {
	global $q_config;
	if($q_config['mqtranslate_services']) {
	/* disabled for meta box
		add_filter('mqtranslate_toolbar',			'qs_toobar');
		add_filter('mqtranslate_modify_editor_js',	'qs_editor_js');
	*/
		add_meta_box('translatediv', __('Translate to','mqtranslate'), 'qs_translate_box', 'post', 'side', 'core');
		add_meta_box('translatediv', __('Translate to','mqtranslate'), 'qs_translate_box', 'page', 'side', 'core');
		
		add_action('mqtranslate_languageColumn',			'qs_translateButtons', 10, 2);
		
		// add plugin page without menu link for users with permission
		if(current_user_can('edit_published_posts')) {
			//add_posts_page(__('Translate','mqtranslate'), __('Translate','mqtranslate'), 'edit_published_posts', 'mqtranslate_services', 'qs_service');
			global $_registered_pages;
			$hookname = get_plugin_page_hookname('mqtranslate_services', 'edit.php');
...
function qs_save() {
	global $q_config;
	if($q_config['mqtranslate_services'])
		update_option('mqtranslate_mqtranslate_services', '1');
	else
		update_option('mqtranslate_mqtranslate_services', '0');
}
...
function qs_config_pre_hook($message) {
	...
	if(isset($_POST['default_language'])) {
		qtrans_checkSetting('mqtranslate_services', true, QT_BOOLEAN);
		qs_load();
		if($q_config['mqtranslate_services']) {
	...
	if(isset($_GET['qs_delete'])) {
		...
		$message = __('Order deleted.','mqtranslate');
	}
	if(isset($_GET['qs_cron'])) {
		qs_cron();
		$message = __('Status updated for all open orders.','mqtranslate');
	}
	return $message;
}
...
?>
			<li><img src="<?php echo trailingslashit(WP_CONTENT_URL).$q_config['flag_location'].$q_config['flag'][$language]; ?>" alt="<?php echo $q_config['language_name'][$language]; ?>"> <a href="edit.php?page=mqtranslate_services&post=<?php echo intval($_REQUEST['post']); ?>&target_language=<?php echo $language; ?>"><?php echo $q_config['language_name'][$language]; ?> <span class="qsprice"></span></a></li>
<?php
		} else {
			echo '<li>'.__('Please save your post first.','mqtranslate').'</li>';
			break;
		}
	}
?>
...
function qs_order_columns($columns) {
	return array(
				'title' => __('Post Title', 'mqtranslate'),
				'service' => __('Service', 'mqtranslate'),
				'source_language' => __('Source Language', 'mqtranslate'),
				'target_language' => __('Target Language', 'mqtranslate'),
				'action' => __('Action', 'mqtranslate')
				);
}
...
?>
	<tr valign="top">
		<th scope="row"><h4><?php _e('Open Orders', 'mqtranslate'); ?></h4></th>
		<td>
...
?>
				<tr>
					<td class="qs_no-bottom-border"><a href="post.php?action=edit&post=<?php echo $order['post_id']; ?>" title="<?php printf(__('Edit %s', 'mqtranslate'),$post->post_title); ?>"><?php echo $post->post_title; ?></a></td>
					<td class="qs_no-bottom-border"><?php if(isset($services[$order['service_id']])): ?><a href="<?php echo $services[$order['service_id']]['service_url']; ?>" title="<?php _e('Website', 'mqtranslate'); ?>"><?php echo $services[$order['service_id']]['service_name']; ?></a><?php endif; ?></td>
					...
					<td class="qs_no-bottom-border"><a class="delete" href="<?php echo add_query_arg('qs_delete', $order['order']['order_id'], $request_uri); ?>#mqtranslate_service_settings">Delete</a></td>
				</tr>
<?php 
...
?>
				<tr class="qs_status">
					<td colspan="5">
						<?php printf(__('Current Status: %s','mqtranslate'), $order['status']); ?>
					</td>
				</tr>
<?php
			}
...
?>
	<tr valign="top">
		<th scope="row" colspan="2">
			<h5><?php _e($service['service_name']);?> ( <a name="qs_service_<?php echo $service['service_id']; ?>" href="<?php echo $service['service_url']; ?>"><?php _e('Website', 'mqtranslate'); ?></a> )</h5>
...
function qs_service() {
	...
	if(!isset($_REQUEST['post'])) {
		...
		printf(__('To translate a post, please go to the <a href="%s">edit posts overview</a>.','mqtranslate'), 'edit.php');
		exit();
	}
	...
	if(!$post) {
		printf(__('Post with id "%s" not found!','mqtranslate'), $post_id);
		return;
	}
	...
	if(sizeof($available_languages)==0) {
		$error = __('The requested Post has no content, no Translation possible.', 'mqtranslate');
	}
	...
	if(in_array($translate_to, $available_languages)) {
		$message = __('The Post already has content for the selected target language. If a translation request is send, the current text for the target language will be overwritten.','mqtranslate');
	}
	...
	if(isset($_POST['service_id'])) {
		...
		if(isset($answer['error'])) {
			$error = sprintf(__('An error occured: %s', 'mqtranslate'), $qs_error_messages[$answer['error']]);
			if($answer['message']!='') {
				$error.='<br />'.sprintf(__('Additional information: %s', 'mqtranslate'), qtrans_useCurrentLanguageIfNotFoundUseDefaultLanguage($answer['message']));
			}
		}
...
?>
<p><?php printf(__('The service returned this message: %s','mqtranslate'), $order_completed_message);?></p>
<?php
		}
?>
<p><?php _e('Feel free to choose an action:', 'mqtranslate'); ?></p>
<ul>
...
<form action="edit.php?page=mqtranslate_services" method="post" id="mqtranslate-services-translate">
<p><?php
	if(sizeof($available_languages)>1) {
		...
		printf(__('Your article is available in multiple languages. If you do not want to translate from %1$s, you can switch to one of the following languages: %2$s', 'mqtranslate'),$q_config['language_name'][$translate_from],$available_languages_names);
	}
?></p>
...
?>
<p><?php _e('Please choose the language you want to translate to:', 'mqtranslate');?></p>
<ul>
<?php 
...
?>
	...
	<p class="submit">
		<input type="submit" name="submit" class="button-primary" value="<?php _e('Continue', 'mqtranslate') ?>" />
	</p>
<?php
	} else {
?>
...
<?php if($confirm): ?>
	...
	<div id="submitboxcontainer" class="metabox-holder">
		<div id="submitdiv" class="postbox">
			<h3 class="hndle"><?php _e('Confirm Order', 'mqtranslate'); ?></h3>
			<div class="inside">
				<p><?php _e('Please confirm your order.', 'mqtranslate'); ?></p>
				<div class="qs_submit"><a class="button-primary" onclick="sendorder();"><?php _e('Confirm Order', 'mqtranslate'); ?></a></div>
			</div>
		</div>
	</div>
<?php else: ?>
...
<div id="qs_boxes" class="metabox-holder">
	<div class="postbox">
		<h3 class="hndle"><?php _e('Translation Service', 'mqtranslate'); ?></h3>
		<div class="inside">
...
?>
<li>
	<label><input type="radio" name="service_id" disabled="disabled" /> <b><?php echo qtrans_useCurrentLanguageIfNotFoundUseDefaultLanguage($service['service_name']); ?></b> ( <a href="<?php echo $service['service_url']; ?>" target="_blank"><?php _e('Website', 'mqtranslate'); ?></a> )</label>
	<p class="error"><?php printf(__('Cannot use this service, not all <a href="%s">required fields</a> filled in for this service.','mqtranslate'), 'options-general.php?page=mqtranslate#qs_service_'.$service_id); ?></p>
	<p class="service_description"><?php echo qtrans_useCurrentLanguageIfNotFoundUseDefaultLanguage($service['service_description']); ?></p>
</li>
<?php
				} else {
?>
<li><label><input type="radio" id="qs_service_<?php echo $service['service_id'];?>" onclick="chooseservice(this.value)" value="<?php echo $service['service_id'];?>" <?php echo $confirm?'disabled="disabled"':'name="service_id"'; ?> /> <b><?php echo qtrans_useCurrentLanguageIfNotFoundUseDefaultLanguage($service['service_name']); ?></b> ( <a href="<?php echo $service['service_url']; ?>" target="_blank"><?php _e('Website', 'mqtranslate'); ?></a> )</label><p class="service_description"><?php echo qtrans_useCurrentLanguageIfNotFoundUseDefaultLanguage($service['service_description']); ?></p></li>
<?php
				}
			}
?>
...
<script type="text/javascript">
	function chooseservice(id) {
		...
		jQuery('#submitdiv .request').html('<?php _e('<p><img src="images/wpspin_light.gif"> Getting Quote...</p>', 'mqtranslate'); ?>');
		jQuery.post(ajaxurl, {
			action: 'qs_quote',
	...
	function sendorder() {
		jQuery("#mqtranslate-services-translate").submit();
	}
...
function qs_quote() {
	...
	$request = array(
			...
			'order_target_language' => $translate_to,
			'order_target_locale' => $q_config['locale'][$translate_to],
			'order_confirm_url' => get_admin_url(null, 'edit.php?page=mqtranslate_services&confirm=1&post='.$_POST['post_id'].'&source_language='.$translate_from.'&target_language='.$translate_to.'&service_id='.$service_id),
			'order_failure_url' => get_admin_url(null, 'edit.php?page=mqtranslate_services&post='.$_POST['post_id'].'&source_language='.$translate_from.'&target_language='.$translate_to.'&service_id='.$service_id)
		);
	$answer = qs_queryQS(QS_QUOTE, $request);
	$price = __('unavailable', 'mqtranslate');
	$currency = '';
	$short = '';
	if(isset($answer['price'])) {
		if($answer['price'] == 0) {
			$price = __('free', 'mqtranslate');
		...
		$content = sprintf(__('<p>Price: %1$s %2$s</p>','mqtranslate'), $currency, $price);
		$short = sprintf(__('~ %1$s %2$s','mqtranslate'), $currency, $price);
		if(!empty($answer['paypalurl'])) {
			...
			$content .= '<div class="qs_submit"><a class="button-primary" onclick="sendorder();">'.__('Request Translation', 'mqtranslate').'</a></div>';
		}
	} else {
		$content = '<p>'.__('An error occured!', 'mqtranslate');
...
function qs_editor_js($content) {
	$content .= "
		init_qs = function(action, id) {
			document.location.href = 'edit.php?page=mqtranslate_services&post=".intval($_REQUEST['post'])."';
		}
		";
	return $content;
}
