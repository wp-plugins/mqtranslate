...
function qtrans_admin_notice_deactivate_plugin($nm,$plugin)
{
	...
	$msg=sprintf(__('Activation of plugin %s deactivated plugin %s since they cannot run simultaneously.','qtranslate'),$qtlink,$link).' ';
	if ($imported)
		$msg.=sprintf(__('The compatible settings from %s have been imported to %s. Further tuning, import, export and reset of options can be done at Settings/Languages configuration page, once %s is running.%sContinue%s','qtranslate'),$nm,$qtnm,$qtnm,'</p><p><a  class="button" href="">','</a>');
	else
		$msg.=sprintf(__('You may import/export compatible settings from %s to %s on Settings/Languages configuration page, once %s is running.%sContinue%s','qtranslate'),$nm,$qtnm,$qtnm,'</p><p><a  class="button" href="">','</a>');
...
function qtrans_admin_notice_plugin_conflict($title,$plugin)
{
	...
	echo '<span style="color:red"><strong>'.__('Error').':</strong></span> '.sprintf(__('plugin %s cannot run concurrently with plugin %s. You may import and export compatible settings between %s and %s on Settings/<a href="%s">Languages</a> configuration page. Then you have to deactivate one of the plugins to continue.','qtranslate'),$me,$link,'mqTranslate',$title,admin_url('options-general.php?page=mqtranslate'));
