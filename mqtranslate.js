...
function qtrans_get_cookie(cname)
{
    var nm = cname + "=";
    ...
    for (var i = 0; i < ca.length; i++) {
        var c = ca[i];
        var p = c.split('=');
        if (p[0].trim() == cname && ca.length >= 2) return p[1].trim();
    }
    return '';
}
...
function qtrans_isArray(obj){ return obj.constructor.toString().indexOf('Array') >= 0; }
...
var qTranslate=function()
{
    var activeLanguage=qtrans_get_cookie('qtrans_edit_language');
	if(!activeLanguage)
	...
	function updateFusedValue(target, newValue, store, activeLanguage)
	{
        ...
        target.value = qtrans_join(store);
	}
	...
	function tagEdit()
	{
		...
		{
			var lang=langs[i];
            newNameFields[lang]=ce('input', {name: 'qtrans_term_'+lang, className: 'hidden', value: name[lang] || ''}, form, true);
		}
	...
	function postEdit()
	{
		...
		var titles = qtrans_split(titleField.value);
		var mlTitleField=ce(titleField.tagName, {name: titleField.name, className: 'hidden', value: titleField.value}, form, true);
		titleField.name += '_edit';
		titleField.value = titles[activeLanguage];
		titleField.onblur = function() {
			updateFusedValue(mlTitleField, this.value, titles, languageSwitch.getActiveLanguage());
		};
		
		var contentField = ge('content');
		if (!contentField)
			contentField = ge('attachment_content');
		if (contentField) {
			var contents = qtrans_split(contentField.value);
			var mlContentField=ce(contentField.tagName, {name: contentField.name, className: 'hidden', value: contentField.value}, form, true);
			contentField.name += '_edit';
			contentField.value = contents[activeLanguage];
			contentField.onblur = function() {
				updateFusedValue(mlContentField, this.value, contents, languageSwitch.getActiveLanguage());
			};
		}
		
		var captionField = ge('attachment_caption');
		if (captionField) {
			var captions = qtrans_split(captionField.value);
			var mlCaptionField = ce(captionField.tagName, { name: captionField.name, className: 'hidden', value: captionField.value }, form, true);
			captionField.name += '_edit';
			captionField.value = captions[activeLanguage];
			captionField.onblur = function() {
				updateFusedValue(mlCaptionField, this.value, captions, languageSwitch.getActiveLanguage());
			};
		}
		...
		onTabSwitch=function()
		{
			...
			if (captionField)
				captionField.value=captions[this.lang];
			setSlugLanguage(this.lang);
			if (window.tinyMCE && tinyMCE.activeEditor && contentField)
    ...
    var langSwitchWrap=ce('ul', {className: 'qtrans-lang-switch-wrap'});
	{
...
function LanguageSwitch(target,initial_language)
{
	...
	{
		var lang=langs[i];
        var tabSwitch=ce ('li', {lang: lang, className: 'qtrans-lang-switch', onclick: switchTab }, target );
