...
function qtrans_ce(tagName, props, pNode, isFirst)
{
...
var qTranslate=function()
{
	...
	function updateFusedValueH(id,value)
	{
		...
		if(h.separator==='<'){
			h.mlContentField.value = qtrans_join_c(h.contents);
		}else{
			h.mlContentField.value = qtrans_join_b(h.contents);
		}
	...
	function tagEdit()
	{
		...
		{
			var lang=langs[i];
			newNameFields[lang]=qtrans_ce('input', {name: 'qtrans_term_'+lang, className: 'hidden', value: name[lang] || ''}, form, true);
		}
